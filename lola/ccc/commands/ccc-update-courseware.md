---
description: "Update the courseware plugin to get the latest modules and improvements."
---
...
if [ "$LOCAL" = "$REMOTE" ]; then
  ...
  NEW_MODULES=$(git -C "$PLUGIN_REPO" diff HEAD..origin/main --name-only --diff-filter=A -- '../skills/*.md' 2>/dev/null)
  ...
  UPDATED_MODULES=$(git -C "$PLUGIN_REPO" diff HEAD..origin/main --name-only --diff-filter=M -- '../skills/*.md' 2>/dev/null)
...
if [ "$LOCAL" = "$REMOTE" ]; then
  ...
  MODULE_COUNT=$(ls "$PLUGIN_REPO/../skills/"*.md 2>/dev/null | grep -v TEMPLATE | wc -l | tr -d ' ')
  echo "  Modules available: $MODULE_COUNT"
  echo ""
  echo "Restart Claude Code to load the updated content, then run"
  echo "/ccc-courseware to see the full catalog."
else
