---
name: learn-21-plugin-marketplace
description: "Plugin Marketplace -- discover, install, and manage plugins (~15 min). Prerequisites: Module 01, Module 09 recommended"
---

# Plugin Marketplace

Discover, install, and manage Claude Code plugins from marketplace registries.
Estimated time: 15 minutes. Prerequisites: Module 01, Module 09 recommended.

Read and follow the module at `../../modules/21-plugin-marketplace.md` step by step.

Start with the Orientation section, then run the Preflight checks.
Skip any step where the prerequisite already exists.
Guide the user through each remaining step, then run Verification.
Finish with the Challenge.

Use `../../.claude/commands/references/context.md` for team-specific values.
