<!-- NEW -->
# Module 27 -- Event-Scale Operations

Estimated time: 30 minutes
Prerequisites: Module 23 (RHDP-Flow MCP), Module 25 (CSV Pipeline), Module 26 (Deployment Intelligence)
...
You'll need:
  ...
  - Module 25 completed (CSV Pipeline working)
  - Module 26 completed (Deployment Intelligence working)
...
```bash
python3 -c "import rhdp_flow_csv" 2>/dev/null && echo "EXISTS: rhdp-flow-csv package" || echo "MISSING: rhdp-flow-csv -- complete Module 25 first"
```

### Check 3: rhdp-flow-intel server

```bash
python3 -c "import rhdp_flow_intel" 2>/dev/null && echo "EXISTS: rhdp-flow-intel package" || echo "MISSING: rhdp-flow-intel -- complete Module 26 first"
```
